    ...
    void Start()
    {
       ...
       used_spaces = new List<KeyValuePair<int, int>>();
        ...
        for ( int j = 0; j < 10; j++)
        {
            ...
            used_spaces.Add(cur);
            Instantiate(MysteryBox, new Vector3((cur.Key - mg.GetCenterX()) * 2, 1, (cur.Value - mg.GetCenterZ()) * 2), MysteryBox.transform.rotation).transform.parent = gameObject.transform;
            this.cur_spawned++;
        }
